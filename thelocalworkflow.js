...
if (args.transcode) {
  ...
  if (args.folder) {
    ...
    // for (var i = 0; i < camfoldersToTranscode.length; i++) {
    //   if (fs.statSync(path.join(args.folder, camfoldersToTranscode[i])).isDirectory()) {
    //     console.log('we think this is a directory: ' + path.join(args.folder, camfoldersToTranscode[i]));
    //     var filesToTranscode = fs.readdirSync(path.join(args.folder, camfoldersToTranscode[i]));
    //     for (var i = 0; i < filesToTranscode.length; i++) {
    //       console.log(filesToTranscode[i]);
    //     }
    //   }

    //
    // }
    // filesToTranscode.forEach((subfolder) => {
    //   var
    //   var filePath = path.join(args.folder, file);
    //   console.log(filePath);
    //   transcode(filePath, crfVal);
