...
function transcode(sourcePath, destinationPath, crfVal, outputWidth, outputHeight){
  ...
  var output = cp.spawnSync(process.env.FFMPEG_PATH, ['-i', sourcePath, '-c:v', 'libx264', '-vf', 'format=yuv420p', '-vf', ('scale='+ outputWidth +':'+outputHeight ), '-preset', 'slow', '-crf', crfVal, '-c:a', 'aac', '-b:a', '128k', destinationPath]
  , {
    stdio: [
