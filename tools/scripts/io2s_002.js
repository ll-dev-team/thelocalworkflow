...
function io2s(segmentArray, sourceFcpxmlPath){
  var offset = 0;
  ...
  parseXmlString(xmlData, (err, data)=>{
    ...
    // fs.writeFileSync('tests/output/fcpxml/json_from_xml.json', JSON.stringify(data.fcpxml.resources[0].media, null, 4));
    ...
    segmentArray.forEach((segment, index)=>{
      ...
      // console.log(theXml);
      var pathForJson = ('/Users/mk/Development/thelocalworkflow/tests/output/fcpxml/sugar_test.xml');
      fs.writeFileSync(pathForJson, theXml);
      console.log("done");
  });
}
